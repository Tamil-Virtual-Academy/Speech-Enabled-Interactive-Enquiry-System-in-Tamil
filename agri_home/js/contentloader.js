  ...
  xhttp_il.onreadystatechange = function(){
    if (xhttp_il.readyState == 4 && xhttp_il.status == 200){
       ...
       for (var i = 2; i < jsonResponse.length; i++) {
          ...
          document.getElementById(targetIdFunc).innerHTML='<img src="images/cropimages/' + imagefile +'" class="images">';
...
function initialize_iterations(){
    ...
    // clearimages();
    $.ajax({
      url: "beginsession.php",
      type: "GET",
      cache: false,
      success: function(result){
          var mysid = result.trim();
          var newloc = "/agri_home/agri_"+mysid+"/iterations.html";
